...
```tsx
const spacex: string = "Are you ready for launch?";
```
...
```tsx
const value = example<string>("valostring");
console.log(value) // irá printar o valor "valostring"
```
...
Você pode conferir os binarios como por exemplo o tsc em ```node_modules/.bin```
...
Por fim rode o arquivo convertido ```node dist/index.js```